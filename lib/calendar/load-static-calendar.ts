/**
 * Load 2026 ATP Calendar from static data (official PDF)
 * 
 * This loads the complete 2026 ATP calendar from the official tournament schedule
 * into the atp_calendar table. This is a one-time load of the official calendar.
 */
...
// 2026 ATP Calendar data from official PDF
const tournaments = [
  // JANUARY 2026
  {
    name: 'United Cup',
...
export async function loadStaticATPCalendar() {
  ...
  for (const tournament of tournaments) {
    ...
    if (error) {
      ...
      errors++;
      errorDetails.push(`${tournament.name}: ${error.message}`);
    } else {
      success++;
    }
  }
